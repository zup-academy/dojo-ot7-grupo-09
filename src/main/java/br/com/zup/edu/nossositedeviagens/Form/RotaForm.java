...
public class RotaForm {
    ...
    public RotaForm(String nome, Long idAeroportoOrigem, Long idAeroportoDestino, Integer duracao) {
        this.nome = nome;
    ...
    public Rota formToModel(AeroportoRepository aeroportoRepository) {
        ...
        if (this.nome.isBlank()) {
            return new Rota(aeroportoOrigem.get().getNome() + aeroportoDestino.get().getNome(),
                    aeroportoOrigem.get(), aeroportoDestino.get(), duracao);
        }

        return new Rota(this.nome, aeroportoOrigem.get(), aeroportoDestino.get(), duracao);
